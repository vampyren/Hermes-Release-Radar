...
def render_page(data: dict[str, Any], state: dict[str, Any]) -> str:
    ...
    def render_category(cat: str, count: int) -> str:
        ...
        return (
            ...
            f'<summary class="cat-summary"><span class="cat-caret">{CHEVRON_RIGHT_SVG}</span><div class="row spread cat-head"><h3>{html.escape(cat)} <span class="muted">{count}</span></h3><div class="row">{new_badge}'
            f'<button onclick="event.stopPropagation();addMarker(\'{html.escape(cat)} reviewed\', \'{marker_commit}\', \'{aid}\')">Mark reviewed</button></div></div></summary>'
...
flushPendingToast(); activateInitialTab(); window.addEventListener('hashchange', activateInitialTab); renderMarkers(); checkHelperStatus();
</script></body></html>"""
