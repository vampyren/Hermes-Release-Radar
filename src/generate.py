...
APP_VERSION_BADGE = (
    f'<span class="app-version" title="Hermes Release Radar app version">{html.escape(APP_VERSION)}</span>'
    if APP_VERSION
    else ""
)
...
def render_history(state: dict[str, Any]) -> str:
    ...
    if not entries:
        ...
        body = "\n".join(cards)
    return f'''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Hermes Release Radar History</title><link rel="icon" type="image/svg+xml" href="data:image/svg+xml,{FAVICON_DATA}"><style>:root{{color-scheme:dark;--bg:#0b1014;--panel:#121a21;--text:#c7d7dc;--muted:#91a4af;--line:#26343d}}*{{box-sizing:border-box}}body{{margin:0;background:#0b1014;color:var(--text);font:15px/1.45 system-ui,-apple-system,Segoe UI,sans-serif}}main{{max-width:1100px;margin:auto;padding:18px}}a{{color:#a8e9ff}}code{{background:#0b1419;border:1px solid var(--line);border-radius:7px;padding:2px 6px}}.topbar{{border-bottom:1px solid var(--line);background:#0b1014cc;position:sticky;top:0}}.topbar main{{display:flex;align-items:center;justify-content:space-between;padding:8px 18px}}.brand{{display:inline-flex;align-items:center;gap:9px;font-weight:800;color:var(--text);text-decoration:none}}.brand .app-icon{{width:30px;height:30px;filter:drop-shadow(0 0 10px #62e6c833)}}.brandwrap{{display:inline-flex;align-items:center;gap:8px;min-width:0;flex-wrap:wrap}}.app-version{{font-size:11px;font-weight:600;color:var(--muted);letter-spacing:.02em;white-space:nowrap;border:1px solid var(--line);border-radius:999px;padding:1px 7px;background:#0d141a}}.card{{background:linear-gradient(180deg,var(--panel),#10171d);border:1px solid var(--line);border-radius:16px;padding:14px;margin:10px 0;box-shadow:0 12px 30px #0005}}.muted{{color:var(--muted)}}summary{{cursor:pointer}}</style></head><body><div class="topbar"><main><span class="brandwrap"><a class="brand" href="index.html" aria-label="Hermes Release Radar home">{APP_ICON_SVG}<span>Hermes Release Radar History</span></a>{APP_VERSION_BADGE}</span></main></div><main><h1>Installed update history</h1><p class="muted">This is the archive of commits that became installed checkpoints. It is separate from review markers.</p>{body}</main></body></html>'''
