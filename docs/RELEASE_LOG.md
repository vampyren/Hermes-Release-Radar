...
- Fixed the History page silently freezing. `archive_if_head_advanced()` only archived an installed range when the stored `baseline_commit` was an ancestor of the current Hermes HEAD. When the stored baseline stops being an ancestor of HEAD (e.g. an upstream history rewrite), every subsequent run took the "not an ancestor" branch, set a `checkpoint_warning`, and did **not** archive or advance the baseline — freezing History at the last ancestor-valid checkpoint (v0.14.0-era) even as the installed Hermes advanced to v0.15/v0.16/v0.17. The warning was never shown on the page, so the freeze was invisible.
- Release Radar now performs **verified trusted-lineage gap recovery**. When baseline and HEAD have genuinely diverged (neither is an ancestor of the other), it records a durable, clearly-labelled **history gap** record (`kind: "gap"`) and advances the baseline — but only when every read-only, fail-closed trust condition holds: (1) both `baseline_commit` and `HEAD` resolve to real commits, (2) HEAD is on the trusted `origin/main` lineage (equals it or is an ancestor of it), and (3) a real merge-base exists. The gap covers `merge-base(baseline, HEAD)..HEAD` with the diverged old baseline, new baseline (HEAD), nearest common ancestor, real `from`/`to` versions only when reliably derivable (otherwise a neutral `Checkpoint <sha>` — never invented), an **approximate** commit count, releases detected in the range, and a sample of commits. Existing history is never deleted, and a second run is idempotent.
- When any trust condition fails — HEAD on a local feature branch / detached commit not on `origin/main`, `origin/main` unresolvable, baseline missing/garbage-collected, or unrelated histories with no merge-base — Release Radar makes **no change**: it does not append a gap, advance or rewrite the baseline, clear review markers, or alter existing history. It sets a specific `checkpoint_warning` naming the reason. The pre-existing rollback case (HEAD behind the stored baseline) keeps its warn-only behavior.
- Gap explanations use neutral, accurate wording ("Stored Release Radar baseline diverged from the current trusted origin/main lineage. This can happen after an upstream history rewrite or when an earlier checkpoint came from another lineage. Exact intermediate installed checkpoints were not recorded.") rather than asserting an upstream rewrite as fact. The History page renders this canonical wording from code for every gap card, so legacy records display the accurate text too without rewriting stored history.
- The History page surfaces a `checkpoint_warning` banner when present and renders gap records distinctly (warn-styled, "History gap", approximate counts), so skipped recoveries and recovered ranges are honest and visible.
...
- Added a small, explicit state cache (`last_official_release_notes`) holding just what is needed to re-render the card later: release name/tag, URL, `published_at`, body excerpt, parsed highlights, tag commit, and the installed-version context. The cache is updated in `main()` (never from a render function) before `save_state()`, and only when a release is currently reachable — so a GitHub fetch error or an up-to-date checkout preserves the existing cache.
- When no newer release is reachable but a cache exists, `#official` now shows the cached card, clearly and honestly labelled ("Last official release" pill). The intro wording adapts to state: "You are up to date; showing the last official release notes for reference." when `behind == 0`, or "No newer official release is reachable; showing the last official release notes for reference." when `behind > 0`. It states it is reference content only and makes no claim about pending commits in either direction (the other tabs remain the source of truth). The current installed version shown is always the live value, never the cached install-time context. When a newer official release becomes reachable, the live card replaces the cache. If there is no cache yet, the existing no-new-release behavior is unchanged.
- Reference-only and fully scoped to `#official`: the cached content does not affect category counts, behind count, risk/recommendation text, `What actually matters`, `Raw categorized commits`, review markers, or `History`. Release Radar still never runs `hermes update` and does not mutate the Hermes checkout.

## 0.4.10-local - 2026-05-31

- Each category in the Raw categorized commits tab is now individually collapsible, the same way the Review markers card works: every category is a native `<details>` with a rotating chevron, open by default, so you can collapse/expand a category on its own without affecting the others. The `<summary>` toggle contains only the category title (no nested interactive controls), and the `Mark reviewed` button is a separate sibling kept on the same header row — so clicking or keyboard-activating `Mark reviewed` saves the marker without toggling the category, and the category title toggles independently. The per-category `Show all` behavior is unchanged. (`Mark reviewed` shows while a category is expanded; collapsing a category hides its actions along with its commits.)
